...
\begin{enumerate}
    \item Perangkat Komputasi : Perangkat pengembangan dan Server lokal. Adapun spesifikasi perangkat komputer yang digunakan adalah sebagai berikut:
    \begin{enumerate}
        \item Prosesor : AMD Ryzen 5 5500U with Radeon Graphics.
        \item Memori (RAM): 20480MB RAM.
        \item Penyimpanan: \textit{Solid State Drive} (SSD).
        \item Konektivitas: \textit{Local Area Network} (LAN) yang terkoneksi dengan terminal biometrik.
    \end{enumerate}
    ...
    \begin{enumerate}
        \item Tipe Perangkat: Hikvision Face Recognition Terminal (seri representatif: DS-K1T342MFX).
        \item Versi \textit{Firmware}: V4.x (Kompatibel dengan perintah dasar \textit{ISAPI}).
...
Lingkungan perangkat lunak yang digunakan disesuaikan dengan pemilihan solusi yang dilakukan pada Bab III. Berikut merupakan spesifikasi perangkat lunak yang digunakan:

\begin{enumerate}
    \item Sistem Operasi : Windows 11 Home Single Language 64-bit (10.0, Build 26200)
    \item \textit{Runtime Environment}: Node.js (versi LTS, minimum v18.x) sebagai \textit{runtime environment} utama.
    ...
    \item Kerangka Kerja Lapisan Aplikasi (\textit{Backend}): NestJS (versi 11.x), digunakan sebagai kerangka kerja pembuatan struktur \textit{middleware}, manajemen \textit{Cron Job} dan memproses data.
    \item Kerangka Kerja Lapisan Presentasi (\textit{Frontend}): Next.js (versi 16.x) berbasis React, digunakan untuk membangun \textit{dashboard} pemantauan \textit{real-time}.
...
Proses Integrasi dan pengembangan sistem didukung oleh beberapa alat bantu utama, yaitu:

\begin{enumerate}
    \item \textit{Visual Studio Code}, digunakan sebagai lingkungan pengembangan terpadu (IDE)
    \item \textit{Postman}, digunakan sebagai alat bantu pengujian antarmuka aplikasi (API) perangkat biometrik gerbang serta API dari \textit{middleware/backend}.
    \item \textit{Docker Desktop}, digunakan sebagai alat bantu manajemen kontainer, volume dan images yang dihasilkan atau digunakan oleh sistem.
    \item \textit{Node Package Manager} (npm), digunakan untuk manajemen \textit{library} node.js yang digunakan dalam pengembangan sistem.
\end{enumerate}
...
\input{listings/docker-config.tex}
...
Seluruh entitas yang telah dirancang pada Bab IV diimplementasikan ke dalam file konfigurasi \texttt{schema.prisma}. Implementasi ini tidak hanya mencakup penyimpanan data mentah dari perangkat keras, tetapi juga tabel pendukung untuk logika bisnis tingkat lanjut. Visualisasi skema integrasi relasional pada basis data PostgreSQL disajikan pada Gambar \ref{fig:relational_sistem}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/relational_sistem.png}
    \caption{Skema Integritas Relasional dan Batasan Aturan Bisnis}
    \label{fig:relational_sistem}
\end{figure}

Cuplikan kode pada Listing \ref{lst:prisma_schema} menunjukkan implementasi tabel utama dan relasinya.

\input{listings/prisma-schema.tex}
...
\input{listings/prisma-enums.tex}
...
\input{listings/prisma-account.tex}
...
\begin{enumerate}
    ...
    \item Penyusunan Struktur Data Biner Manual: Menghadapi limitasi \textit{firmware} Hikvision dalam membaca pemisah (\textit{boundary}) pengunggahan data biner, \textit{middleware} telah merangkai struktur \textit{multipart} secara manual pada level \textit{byte} sebagai upaya maksimal untuk mengakomodasi format pengunggahan foto wajah. Meskipun batasan kompatibilitas \textit{firmware} perangkat pada akhirnya menolak injeksi visual ini, implementasi arsitektur ini dipersiapkan agar sistem siap mendukung sinkronisasi foto apabila terdapat pembaruan (\textit{update}) \textit{firmware} mesin di masa mendatang.
\end{enumerate}

\input{listings/isapi-multipart.tex}

\subsection{Implementasi Mekanisme Sinkronisasi}
Sinkronisasi identitas biometrik dan penarikan log dikelola oleh \texttt{CronService}. Sistem menerapkan strategi koordinasi tugas untuk menyeimbangkan performa dan stabilitas jaringan. Proses penarikan log dari gerbang beroperasi secara paralel (\textit{asynchronous concurrent}) guna menjamin efisiensi waktu eksekusi. Sebaliknya, proses distribusi data pendaftaran karyawan baru dilakukan melalui titik integrasi terminal untuk kemudian disinkronisasikan ke seluruh unit gerbang melalui ekosistem jaringan perangkat keras yang tersedia (yang pada kondisi operasional saat ini memerlukan inisiasi sinkronisasi manual melalui tombol kendali pada lapisan fisik terminal).

Algoritma utama yang digunakan adalah \textit{Mark \& Sweep}, di mana sistem memperbarui stempel waktu pada fase penandaan, dilanjutkan dengan pembersihan identitas usang pada fase penyisiran sebagaimana ditunjukkan pada Listing \ref{lst:logic_sweep}. 
Dalam pendaftaran identitas, sistem menerapkan logika dua tahap: sinkronisasi identitas tekstual (Nama dan ID) terlebih dahulu, kemudian dilanjutkan dengan percobaan pengunggahan foto. Logika toleransi kesalahan (\textit{fault tolerance}) ini dirancang agar proses pendaftaran karyawan tetap dianggap berhasil dan tersimpan di basis data peladen, meskipun tahap pengunggahan foto wajah pada akhirnya ditolak oleh perangkat keras akibat batasan kompatibilitas protokol. Peladen bertindak sebagai pusat kebenaran data (\textit{Single Source of Truth}), menyimpan foto tersebut secara lokal sebagai arsip visual pada antarmuka \textit{dashboard} untuk menutupi kelemahan perangkat fisik.
Manajemen siklus hidup data dilengkapi dengan prosedur penghapusan berlapis: mencabut akses pada memori perangkat, menghapus aset foto di sistem berkas peladen, dan membersihkan rekaman basis data. Selain itu, untuk menjamin tidak ada log yang terabaikan akibat fluktuasi jaringan, penarikan data menggunakan strategi jendela waktu mundur selama delapan jam. Integritas penyerapan log diperkuat melalui penyaringan data menggunakan nomor seri (\textit{serialNo}) unik guna mencegah terjadinya duplikasi rekaman riwayat akses pada basis data meskipun terjadi tumpang tindih siklus penarikan data.

\input{listings/logic-sweep.tex}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/impl_dashboard.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/impl_management.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/impl_verification.png}
