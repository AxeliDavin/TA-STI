...
% ============================================================================================
\cleardoublepage
\chapter{ANALISIS MASALAH DAN KEBUTUHAN}
\label{chap:analisis}
Bab ini membahas secara mendalam analisis masalah yang terdapat pada sistem manajemen akses fisik saat ini di Gedung ITB Innovation Park (IIP), analisis kebutuhan untuk menyelesaikan masalah tersebut, serta analisis pemilihan solusi teknologi yang diusulkan. Alur pembahasan pada bab ini disusun berdasarkan hasil investigasi lapangan yang dilakukan secara bertahap guna memastikan solusi yang dirancang tepat sasaran.

\section{Metodologi Pengumpulan Data}
Analisis dalam penelitian ini didasarkan pada data primer dan sekunder yang dikumpulkan melalui tiga metode utama. Proses ini merupakan perwujudan dari tahap \textit{Empathize} dan \textit{Define} dalam kerangka kerja \textit{Design Thinking} yang diadopsi.

\subsection{Observasi Fisik dan Survei Lapangan}
Investigasi dimulai dengan melakukan peninjauan langsung terhadap infrastruktur kontrol akses di Gedung IIP. Observasi difokuskan pada pemetaan sebaran 6 (enam) unit terminal biometrik di area lobi utama serta jalur komunikasi kabel LAN yang menghubungkan perangkat keras dengan stasiun kerja resepsionis. Melalui survei ini, diidentifikasi adanya keterbatasan visibilitas data karena sistem yang berjalan masih bersifat lokal dan terisolasi (\textit{standalone}).

\subsection{Wawancara Mendalam (\textit{In-depth Interview})}
Untuk menggali kebutuhan operasional secara riil, dilakukan serangkaian wawancara terstruktur dengan pihak pengelola gedung dan perwakilan dari Direktorat Kawasan Sains dan Teknologi - DKST. Wawancara dilaksanakan dalam 5 (lima) sesi utama yang mencakup pembahasan mengenai alur masuk penghuni, koordinasi teknis instalasi, integrasi BMS, hingga kebijakan privasi visual. Seluruh transkrip dan dokumentasi foto dari sesi wawancara ini dapat dilihat pada Lampiran A hingga Lampiran E.

\subsection{Studi Dokumentasi Teknis}
Selain data lapangan, dilakukan analisis terhadap dokumentasi resmi protokol \textit{Intelligent Security API} (ISAPI) dari vendor Hikvision. Studi ini bertujuan untuk memetakan skema JSON/XML yang diperlukan untuk melakukan penarikan log dan pendaftaran subjek secara nirkabel, guna menjamin interoperabilitas antara perangkat lunak kustom dengan perangkat keras industrial.
...
Seluruh 6 terminal di lobi terhubung melalui jaringan LAN lokal ke satu unit komputer di area lobi yang bertindak sebagai PC Resepsionis. Komputer ini menjalankan perangkat lunak \textit{proprietary} vendor (iVMS-4200) yang berfungsi sebagai server lokal untuk konfigurasi perangkat dan penarikan data log. Visualisasi arsitektur jaringan yang terfragmentasi saat ini dapat dilihat pada Gambar \ref{fig:arsitektur_jaringan_sebelum}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{images/arsitektur_jaringan_sebelum.png}
    \caption{Arsitektur Jaringan dan Aliran Data yang Terisolasi (\textit{Data Silo})}
    \label{fig:arsitektur_jaringan_sebelum}
\end{figure}

(Catatan Batasan: Meskipun terdapat akses masuk melalui area basemen menggunakan teknologi sidik jari, area tersebut tidak termasuk dalam ruang lingkup penelitian ini dan tidak diberikan solusi dalam pengembangan sistem pada tugas akhir ini. Fokus penelitian sepenuhnya diarahkan pada integrasi web untuk keenam terminal di area lobi utama).
...
\begin{enumerate}
    ...
    \item Kesulitan Investigasi Kejadian Anomali:   ka terjadi kejadian akses yang ditolak (seperti wajah tidak dikenali), pengelola tidak dapat meninjau bukti rekaman visual (snapshot) kejadian tersebut dari jarak jauh. Investigasi terhadap anomali mengharuskan petugas keamanan untuk memeriksa langsung melalui PC Resepsionis di lobi, yang mana proses ini sangat tidak efisien untuk operasional gedung skala besar.
